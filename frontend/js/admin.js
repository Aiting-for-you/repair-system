...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSchoolsTable(schools) {
        ...
        if (schools.length === 0) {
            schoolsTableBody.innerHTML = '<tr><td colspan="2">暂无学校信息</td></tr>';
            return;
        }
        schools.forEach(school => {
            ...
            row.innerHTML = `
                <td>${school.id}</td>
                <td>${school.name}</td>
                <!-- <td><button class="button edit-btn">编辑</button> <button class="button delete-btn">删除</button></td> -->
            `;
        });
    }
    ...
    async function handleBatchExportExcel() {
        // 新增：弹窗选择学校和时间段
        let schoolId = prompt('请输入学校ID（留空为全部）');
        let start = prompt('请输入起始日期（格式：2024-01-01，留空为不限）');
        let end = prompt('请输入结束日期（格式：2024-12-31，留空为不限）');
        let params = [];
    ...
    exportBatchExcelBtn.addEventListener('click', handleBatchExportExcel);
